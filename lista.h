...
/**COPIA LOS ELEMENTOS DE UNA LISTA A OTRA, PASADA IMPLICITAMENTE**/
template <class E>
...
/**RETORNA EL ELEMENTO QUE ESTA EN LA POSICION (pos) DE LA LISTA **/
template <class E>
...
/**INSERTA UN ELEMENTOS AL FINAL DE LA LISTA (ULTIMA POSICION)**/
template <class E>
...
/**ELIMINA EL ELEMENTO QUE ESTA EN LA POSICION (pos) DE LA LISTA **/
template <class E>
...
bool LISTA<E>::operator==(const LISTA<E> &l2) 
{
	...
	if( this->cantidad != l2.cantidad ) return (false);
  	else
	{
		if(  this->ultimo->get_elemento() != l2.ultimo->get_elemento() ) return (false);
		else
		{
			aux1 = this->primero;
...
void LISTA<E>::operator=(const LISTA<E> &l2) 
{
	...
	{	
		...
		while( aux  != NULL )
		{
			this->insertar_al_final( aux->get_elemento() ); 		
			aux = aux->proximo();		
		}
	}else                                        
	{                                            
